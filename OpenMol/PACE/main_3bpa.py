...
from dataset.Pyg3BPA import Pyg3BPA
from dataset.Preprocess_3BPA import Preprocess3BPA
...
def main():
    ...
    ### Load data
    dataset = Pyg3BPA(
        root="./dataset/3bpa/pyg_data/", name="train_300K"
    )
    split_idx = dataset.get_idx_split(
        len(dataset.data.y), valid_fraction=0.1, seed=args.seed
    )
    train_dataset = Preprocess3BPA(dataset[split_idx["train"]], args.cutoff)
    valid_dataset = Preprocess3BPA(dataset[split_idx["valid"]], args.cutoff)
    ...
    test_dataset_1 = Pyg3BPA(
        root="./dataset/3bpa/pyg_data/", name="test_300K"
    )
    test_dataset_1 = Preprocess3BPA(test_dataset_1, args.cutoff)
    test_loader_1 = DataLoader(
        test_dataset_1, batch_size=args.val_batch_size, shuffle=False, drop_last=False
    )
    test_dataset_2 = Pyg3BPA(
        root="./dataset/3bpa/pyg_data/", name="test_600K"
    )
    test_dataset_2 = Preprocess3BPA(test_dataset_2, args.cutoff)
    test_loader_2 = DataLoader(
        test_dataset_2, batch_size=args.val_batch_size, shuffle=False, drop_last=False
    )
    test_dataset_3 = Pyg3BPA(
        root="./dataset/3bpa/pyg_data/", name="test_1200K"
    )
    test_dataset_3 = Preprocess3BPA(test_dataset_3, args.cutoff)
    test_loader_3 = DataLoader(
        test_dataset_3, batch_size=args.val_batch_size, shuffle=False, drop_last=False
    )
    test_dataset_4 = Pyg3BPA(
        root="./dataset/3bpa/pyg_data/", name="test_dih"
    )
    test_dataset_4 = Preprocess3BPA(test_dataset_4, args.cutoff)
    test_loader_4 = DataLoader(
        test_dataset_4, batch_size=args.batch_size, shuffle=False, drop_last=False
    )
    ...
    test_metrics_3 = evaluate(model, test_loader_3, loss_fn, None, device)
    test_metrics_4 = evaluate(model, test_loader_4, loss_fn, None, device)

    add_row_3bpa(table, test_metrics_1, "300K")
    add_row_3bpa(table, test_metrics_2, "600K")
    add_row_3bpa(table, test_metrics_3, "1200K")
    add_row_3bpa(table, test_metrics_4, "dih")
    logging.info("\n" + str(table))
