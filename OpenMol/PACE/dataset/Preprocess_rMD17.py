...
if __name__ == "__main__":
    from PygrMD17 import rMD17

    mode = "train_01"
    pyg_dataset = rMD17(root="./rmd17/pyg_dataset/", name="benzene", mode=mode)
    split_idx = pyg_dataset.get_idx_split(
        len(pyg_dataset.data.y), valid_fraction=0.05, seed=3
    )
    train_dataset = pyg_dataset[split_idx["train"]]
    print(train_dataset)
    dataset = rMD17Preprocess(train_dataset, cutoff=5)
    print(dataset)
    print(dataset[0])
