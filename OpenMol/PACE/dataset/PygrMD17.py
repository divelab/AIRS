...
class rMD17(InMemoryDataset):
    def __init__(
        ...
        self.name = name
        self.mode = mode
        self.processed_folder = osp.join(root, name, mode)

        super(rMD17, self).__init__(
    ...
    def raw_file_names(self):
        return osp.join(self.name, self.mode + ".xyz")
...
if __name__ == "__main__":
    mode = "train_01"
    dataset = rMD17(root="./rmd17/pyg_data/", name="benzene", mode=mode)
    print(dataset)
    ...
    if mode[:5] == "train":
        split_idx = dataset.get_idx_split(
            len(dataset.data.y), valid_fraction=0.05, seed=3
        )
