...
class Contraction(torch.nn.Module):
            ...
            if i == correlation:
                ...
                self.graph_opt_main = opt_einsum_fx.optimize_einsums_full(
                    ...
                    example_inputs=(
                        ...
                        torch.randn((num_examples, self.num_features, num_ell)),
                        torch.randn((num_examples, num_elements)),
                    ),
                )
                ...
                graph_opt_features = opt_einsum_fx.optimize_einsums_full(
                    ...
                    example_inputs=(
                        torch.randn(
                            [num_examples, self.num_features, num_equivariance]
                            + [num_ell] * i
                        ).squeeze(2),
                        torch.randn((num_examples, self.num_features, num_ell)),
                    ),
    ...
    def forward(self, x, y):
        ...
        for i, (weight, contract_weights, contract_features) in enumerate(
            ...
            out = contract_features(c_tensor, x[i])
