...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', default='mamba_test',
    ...
    if args.conditions_path is not None:
        ...
        with open(args.conditions_path, 'r') as file:
            data = file.readlines()
        first_tokens = [line.strip() for line in data]
    ...
    num_samples = args.repeats  # Number of samples to generate for each first token
    ...
    for idx, first_token in enumerate(first_tokens):
        ...
        out = generate(
            first_token_ids)  # out.sequences, out.scores
        ...
        with open(f'{args.save_path}/gen_mol_{idx}.txt', 'w') as f:
            for sample in samples:
                if sample.startswith(first_token):
                    gen_sample = sample[len(first_token):]
                    f.write(gen_sample + '\n')
                else:
