...
class Transolver_block(nn.Module):
    ...
    def __init__(
        ...
        self.adaLN_modulation_attn = MLP(cond_dim, hidden_dim * mlp_ratio, hidden_dim * 3, n_layers=0, res=False, act=act)
        self.adaLN_modulation_mlp = MLP(cond_dim, hidden_dim * mlp_ratio, hidden_dim * 3, n_layers=0, res=False, act=act)
        if self.last_layer:
            self.ln_3 = nn.LayerNorm(hidden_dim)
            self.mlp2 = nn.Linear(hidden_dim, out_dim)

    def forward(self, fx, emb):
        shift_msa, scale_msa, gate_msa = self.adaLN_modulation_attn(emb).chunk(3, dim=1) 
        fx = gate_msa.unsqueeze(1) * self.Attn(modulate(self.ln_1(fx), shift_msa, scale_msa)) + fx
        shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation_mlp(emb).chunk(3, dim=1)
        fx = gate_mlp.unsqueeze(1) * self.mlp(modulate(self.ln_2(fx), shift_mlp, scale_mlp)) + fx
        if self.last_layer:
