...
class SpectralConv2d(nn.Module):
    def __init__(self, dim, cond_dim, activation, n_modes, forecast_ff, backcast_ff,
        ...
        self.cond_emb = nn.Sequential(
            ...
            self.activation,
            FreqLinear(in_channel=2 * dim, modes=n_modes),
        )
        self.dt_emb = nn.Linear(1, dim)
        ...
        if not self.fourier_weight:
            ...
            for _ in range(2):
                weight = torch.FloatTensor(dim, dim, n_modes, 2)
        ...
        if not self.backcast_ff:
            self.backcast_ff = FeedForward(
                dim, factor, ff_weight_norm, n_ff_layers, layer_norm, dropout)
...
class Experts(nn.Module):
    def __init__(self, num_experts, gate_channels, in_dim, out_dim, cond_dim, activation, n_modes,
                 forecast_ff, backcast_ff,
                 fourier_weight, factor, ff_weight_norm,
                 n_ff_layers, layer_norm, dropout, mode):
        super().__init__()
        ...
        if self.activation is None:
            raise NotImplementedError(f"Activation {activation} not implemented")
        expert_init = lambda: SpectralConv2d(
            in_dim, cond_dim, activation, n_modes, forecast_ff, backcast_ff,
            fourier_weight, factor, ff_weight_norm,
            n_ff_layers, layer_norm, dropout, mode
        )
        ...
        self.gate = nn.Sequential(
            nn.Linear(1, gate_channels),
            self.activation,
            nn.Linear(gate_channels, num_experts),
            nn.Softmax(dim=1),
        )

    def forward(self, x, emb):
        dt, emb = emb
        weights = self.gate(dt).T
        h = 0
        for gate, expert in zip(weights, self.experts):
            while len(gate.shape) < len(x.shape):
                gate = gate[..., None]
            h = h + gate * expert(x, [dt, emb])
        return h, None


class FNOFactorized2DBlock(nn.Module):
    def __init__(self, modes, width, num_experts=2, gate_channels=12,
                 input_dim=12, output_dim=12, cond_dim=1, activation='gelu',
        ...
        for _ in range(n_layers):
            self.spectral_layers.append(Experts(num_experts=num_experts,
