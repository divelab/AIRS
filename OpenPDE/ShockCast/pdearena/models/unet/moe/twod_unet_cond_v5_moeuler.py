...
class ExpertBlock(nn.Module):
    ...
    def __init__(
        ...
        if cond_channels > 0:
            ...
            self.cond_emb = None
        self.dt_emb = nn.Linear(1, out_channels)

    def forward(self, x: torch.Tensor, emb: List[torch.Tensor]):
        # First convolution layer
        h = self.conv1(self.activation(self.norm1(x)))
        dt, emb = emb
        dt_emb = self.dt_emb(dt)
        while len(dt_emb.shape) < len(h.shape):
            dt_emb = dt_emb[..., None]
        if emb is not None:
...
class UpBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, emb: List[torch.Tensor]) -> torch.Tensor:
        x = self.res(x, emb)
        x = self.attn(x)
        return x
