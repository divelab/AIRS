...
from .condition_utils import fourier_embedding
import numpy as np


class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, cond_channels: int, padding_mode, num_groups=1, norm: bool = True, activation="gelu", use_scale_shift_norm: bool = False,) -> None:
        super().__init__()
...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, in0_channels, cond_channels, padding_mode, num_blocks, residual, pool, avg, num_groups=1, norm: bool = True, activation="gelu", first=False, disentangle=True, use_scale_shift_norm: bool = False) -> None:
        super().__init__()
        ...
        for block in range(num_blocks):
            ...
            self.conv.append(ConvBlock(in_c, out_channels, cond_channels, padding_mode, num_groups, norm, activation, use_scale_shift_norm=use_scale_shift_norm))
        if residual:
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, cond_channels, mult, padding_mode, num_blocks, residual, interp, interp_mode, num_groups=1, norm: bool = True, activation="gelu", use_scale_shift_norm: bool = False) -> None:
        super().__init__()
        self.channels = in_channels, out_channels
        self.residual = residual
        self.num_blocks = num_blocks
        ...
        self.conv = nn.ModuleList()
        for block in range(num_blocks):
            self.conv.append(ConvBlock(in0_conv if block == 0 else out_channels, out_channels, cond_channels, padding_mode, num_groups, norm, activation, use_scale_shift_norm=use_scale_shift_norm))
        if residual:
...
class sinenet(nn.Module):
    ...
    def __init__(
        ...
        down_args = dict(cond_channels=time_embed_dim, norm=norm, activation=activation, residual=residual, padding_mode=padding_mode, num_blocks=num_blocks, disentangle=disentangle, pool=down_pool, avg=avg_pool, in0_channels=n_channels, use_scale_shift_norm=use_scale_shift_norm)
        up_args = dict(cond_channels=time_embed_dim, norm=norm, activation=activation, mult=mult, residual=residual, padding_mode=padding_mode, num_blocks=num_blocks, interp=up_interpolation, interp_mode=interpolation_mode, use_scale_shift_norm=use_scale_shift_norm)
    ...
    def forward(self, x, time, z=None):
        assert x.dim() == 5
        orig_shape = x.shape
        ...
        for stack in range(self.num_waves):
            ...
            for i in range(self.num_layers):
                dx, h = self.down[stack][i](dx, xs[-1], emb)
                xs.append(h)
            x = xs.pop(-1)
            for i in range(self.num_layers):
                x = self.up[stack][i](x, xs.pop(-1), emb)
            if self.residual:
                x = x0 + x
