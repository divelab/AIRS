...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, padding_mode, num_groups=1, norm: bool = True, activation="gelu") -> None:
        ...
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=4, padding_mode=padding_mode, dilation=4)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=4, padding_mode=padding_mode, dilation=4)
        if norm:
...
def dilated_pool(x):
    B, C, H, W = x.shape 
    x = x.reshape(B, C, H // 8, 2, 4, W // 8, 2, 4)
    x = x.mean(dim=(-2, -5))
...
class circular_interpolate(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 1, 3, 5, 2, 4).reshape(B, C*16, H//4, W//4)
        x = torch.nn.functional.pad(x, (2, 2, 2, 2), mode='circular')
        x = torch.nn.functional.interpolate(x, scale_factor=2, mode=self.mode)
        x = x[..., 4:-4, 4:-4]
        x = x.reshape(B, C, 4, 4, H//2, W//2)
        x = x.permute(0, 1, 4, 2, 5, 3).reshape(B, C, H*2, W*2)
        return x
...
class sinenet(nn.Module):
    ...
    def __init__(
        ...
        self.image_proj = nn.Conv2d(insize, n_channels, kernel_size=3, padding=4, padding_mode=padding_mode, dilation=4)
        ...
        self.final = nn.Conv2d(n_channels, out_channels, kernel_size=3, padding=4, padding_mode=padding_mode, dilation=4)
