...
from .ocpmodels.scn.smearing import (
    GaussianSmearing,
...
class EquiformerV2_Eband_Pool(BaseModel):
    """
    ...
    def __init__(
        ...
        for i in range(self.num_resolutions):
            self.global_SO3_rotation.append(SO3_Rotation(self.lmax_list[i]))
        ...
        self.coef_proj = SO3_LinearV2((self.time_cond * 2 + 1) * 2, self.sphere_channels, lmax=self.lmax_list[0])
 
        if self.efield_cond:
            self.norm_type = self.norm_type + '_cond'
            self.cond_channels_list = [(self.time_cond + self.time_future) * 10, self.sphere_channels, self.sphere_channels * (max(self.lmax_list) + 1) * 2]
            ...
            self.norm = get_cond_normalization_layer(self.norm_type, lmax=max(self.lmax_list), num_channels=self.sphere_channels, 
                                                     mappingReduced=self.mappingReduced, cond_channels_list=self.cond_channels_list)
        else:
            self.cond_channels_list=None
        ...
        self.eband_attn_blocks = nn.ModuleList()
        # self.global_in_projs = nn.ModuleList()
        for i in range(self.num_layers):
            block = TransBlockV2(
                ...
                cond_channels_list=self.cond_channels_list
            )
            ...
            if self.use_eband_interaction and i < self.num_layers - 1:
                ...
                self.global_attn_blocks.append(block)
    ...
    def forward(self, batch_data, max_eband_samples=None):
        ...
        if self.efield_cond:
            # efield = self.efield_emb(global_data.efield)
        ...
        sampled_num_atoms = coef_cond.shape[1]
        ...
        for i in range(self.num_resolutions):
            ...
            self.SO3_rotation[i].set_wigner(global_edge_rot_mat[sampled_eband_edge_batch])
        ...
        coef_emb = coef_cond.permute(1, 2, 3, 0).reshape(sampled_num_atoms, 9, 2 * num_conds)
        x = SO3_Embedding(
            sampled_num_atoms,
            self.lmax_list,
            num_conds * 2,
        ...
        for i in range(self.num_layers):
            ...
            if self.use_eband_interaction and i < self.num_layers - 1:
                ...
                x_global = self.global_attn_blocks[i](
                    ...
                    global_edge_index=global_edge_index,
                    batch=global_data.batch,    # for GraphDropPath,
                    cond_input=efield,
                    cond_batch=global_data.batch,
        ...
        # out_coef_complex = self.prepare_output_coefs(coefs.embedding[:, :9, :], num_bands=num_bands)
        out_coef_complex = coefs.embedding[:, :9, :].reshape(sampled_num_atoms, 9, 2, self.time_future, 2).permute(4, 3, 0, 1, 2)
