...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        self.so2_conv_1 = SO2_Convolution(
            3 * self.sphere_channels if self.use_dm_feat else 2 * self.sphere_channels,
        ...
        if self.use_dm_feat:
            self.dm_off_diag_proj = o3.Linear(
                # o3.Irreps(f'{9 * 33}x0e+{17 * 33}x1o+{13 * 33}x2e+{5 * 33}x3o+{1 * 33}x4e'),
                o3.Irreps(f'{9 * dm_dim}x0e+{17 * dm_dim}x1e+{13 * dm_dim}x2e+{5 * dm_dim}x3e+{1 * dm_dim}x4e'),
    ...
    def forward(
        ...
        if self.use_atom_edge_embedding:
            ...
            x_edge = torch.cat((global_edge_distance, source_embedding, target_embedding), dim=1)
        else:
            x_edge = global_edge_distance  
        ...
        if self.use_dm_feat:
            dm_off_diag = self.dm_off_diag_proj(dm_off_diag) # E_global, C
            dm_off_diag_split = dm_off_diag.split([self.sphere_channels * d for d in [1, 3, 5, 7, 9]], dim=-1)
            dm_off_diag = torch.cat([feat.reshape(feat.shape[0], self.sphere_channels, -1) for feat in dm_off_diag_split], dim=-1).transpose(1, 2)

        # x_message_data = torch.cat((x_source.embedding, x_target.embedding), dim=2)
        if self.use_dm_feat:
            # x_message_data = torch.zeros(x_source.embedding.shape[0], 25, 3 * self.sphere_channels).to(x_source.embedding.device)
            # x_message_data[:, :9, :self.sphere_channels] = x_source.embedding
            # x_message_data[:, :9, self.sphere_channels : self.sphere_channels * 2] = x_target.embedding
            # x_message_data[:, :, self.sphere_channels * 2 :] = dm_feat[eband_edge_batch]
            if eband_edge_batch is None:
                x_message_data = torch.cat((x_source.embedding, x_target.embedding, dm_off_diag), dim=2)
            else:
                x_message_data = torch.cat((x_source.embedding, x_target.embedding, dm_off_diag[eband_edge_batch]), dim=2)
        else:
            x_message_data = torch.cat((x_source.embedding, x_target.embedding), dim=2)
        ...
        x_message = SO3_Embedding(
            ...
            x_target.num_channels * 3 if self.use_dm_feat else x_target.num_channels * 2, 
        ...
        if self.use_s2_act_attn:
            x_message = self.so2_conv_1(x_message, x_edge, eband_edge_batch)
        else:
            x_message, x_0_extra = self.so2_conv_1(x_message, x_edge, eband_edge_batch)
        ...
        if self.use_s2_act_attn:
            x_message, x_0_extra = self.so2_conv_2(x_message, x_edge, eband_edge_batch)
        else:
            x_message = self.so2_conv_2(x_message, x_edge, eband_edge_batch) # when radfunc is None, eband_edge_batch just to provide number of edges if it is not None 
...
class TransBlockV2(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_cond:
            self.cond_channels_list = cond_channels_list

        max_lmax = max(lmax_list)
        
        if self.use_attn:
            if self.use_cond:
                self.norm_1 = get_cond_normalization_layer(self.norm_type, lmax=max_lmax, num_channels=sphere_channels, 
                                                           mappingReduced=mappingReduced, cond_channels_list=self.cond_channels_list)
            else:
                self.norm_1 = get_normalization_layer(self.norm_type, lmax=max_lmax, num_channels=sphere_channels)
        ...
        if self.use_ffn:
            if self.use_cond:
                self.norm_2 = get_cond_normalization_layer(self.norm_type, lmax=max_lmax, num_channels=sphere_channels, 
                                                           mappingReduced=mappingReduced, cond_channels_list=self.cond_channels_list)
            else:
                self.norm_2 = get_normalization_layer(self.norm_type, lmax=max_lmax, num_channels=sphere_channels)
    ...
    def forward(
        ...
        cond_input=None,  # for conditioning
        cond_batch=None,
        eband_edge_batch=None,
        dm_off_diag=None
    ):
        ...
        if self.use_attn:
            ...
            if self.use_cond:
                output_embedding.embedding = self.norm_1(
        ...
        if self.use_attn and not self.use_ffn:
            return output_embedding

        x_res = output_embedding.embedding
        if self.use_cond:
            output_embedding.embedding = self.norm_2(
                output_embedding.embedding, 
                cond_input=cond_input, 
                # pf_cond=pf_cond, 
                cond_batch=cond_batch
