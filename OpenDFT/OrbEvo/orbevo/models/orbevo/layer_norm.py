...
def get_cond_normalization_layer(norm_type, lmax, num_channels, mappingReduced, cond_channels_list, eps=1e-5, affine=True, normalization='component'):
    assert norm_type in ['rms_norm_sh_cond', 'rms_norm_sh_inv_cond', 'rms_norm_sh_zero_cond']
    if norm_type == 'rms_norm_sh_cond':
        norm_class = EquivariantRMSNormArraySphericalHarmonicsV2Cond
    elif norm_type == 'rms_norm_sh_inv_cond':
        norm_class = EquivariantRMSNormArraySphericalHarmonicsV2InvCond
    elif norm_type == 'rms_norm_sh_zero_cond':
        norm_class = EquivariantRMSNormArraySphericalHarmonicsV2CondZero
    else:
        raise ValueError
    return norm_class(lmax, num_channels, mappingReduced, cond_channels_list, eps, affine, normalization)
...
class EquivariantRMSNormArraySphericalHarmonicsV2Cond(EquivariantRMSNormArraySphericalHarmonicsV2):
    '''
        1. Normalize across all m components from degrees L >= 0.
        2. Expand weights and multiply with normalized feature to prevent slicing and concatenation.
    '''
    # def __init__(self, lmax, num_channels, cond_channels_list, eps=1e-5, affine=True, normalization='component', centering=True, std_balance_degrees=True):
    def __init__(self, lmax, num_channels, mappingReduced, cond_channels_list, eps=1e-5, affine=True, normalization='component', centering=True, std_balance_degrees=True):
        ...
        super().__init__(lmax=lmax, 
                         ...
                         centering=centering, 
                         std_balance_degrees=std_balance_degrees)
        
        self.mappingReduced = mappingReduced
        # self.cond_channels_list = [50 * 160, 128, 128]
        self.cond_channels_list = cond_channels_list # [160, 128, 128 * (self.lmax + 1) * 2]

        self.cond_func = RadialFunction(self.cond_channels_list)
        # self.pf_cond_func = RadialFunction([self.num_channels, self.cond_channels_list[1], (self.lmax + 2) * self.num_channels])
        self.m_0_num_coefficients = self.mappingReduced.m_size[0] 
        self.m_all_num_coefficents = len(self.mappingReduced.l_harmonic)

    # Reshape the embedding m -> l. From so3.py
    def _l_primary(self, embedding):
        embedding = torch.einsum("nac, ab -> nbc", embedding, self.mappingReduced.to_m)
        return embedding
    ...
    def forward(self, 
        ...
        '''
            Assume input is of shape [N, sphere_basis, C]
            cond_input: B, cond_emb_dim
            pf_cond: B, cond_emb_dim
            cond_batch: N
        '''
        node_output = super().forward(node_input=node_input.float())
        cond_feat = self.cond_func(cond_input) # B, (L_max + 2) * C
        cond_feat = cond_feat.reshape(cond_feat.shape[0], (self.lmax + 1) * 2, self.num_channels)
        scale, bias_m_0 = cond_feat.split([self.lmax + 1, self.lmax + 1], dim=1)

        scale = torch.repeat_interleave(scale, torch.arange(self.lmax + 1).to(cond_feat.device) * 2 + 1, dim=1)

        bias_m_0 = bias_m_0.reshape(bias_m_0.shape[0], self.m_0_num_coefficients, self.num_channels)
        bias_m_pad = torch.zeros((
            bias_m_0.shape[0], 
            (self.m_all_num_coefficents - self.m_0_num_coefficients), 
            self.num_channels), 
            device=bias_m_0.device)
        bias_m_all = torch.cat((bias_m_0, bias_m_pad), dim=1)
        # Reshape the spherical harmonics based on l (degree)
        bias_m_all = self._l_primary(bias_m_all)

        output = node_output * scale[cond_batch] + bias_m_all[cond_batch]

        return output


class EquivariantRMSNormArraySphericalHarmonicsV2CondZero(EquivariantRMSNormArraySphericalHarmonicsV2):
    '''
        1. Normalize across all m components from degrees L >= 0.
        2. Expand weights and multiply with normalized feature to prevent slicing and concatenation.
    '''
    ...
    def __init__(self, lmax, num_channels, mappingReduced, cond_channels_list, eps=1e-5, affine=True, normalization='component', centering=True, std_balance_degrees=True):
        ...
        self.cond_func = RadialFunction(self.cond_channels_list)
        self.cond_gate = nn.Linear(self.cond_channels_list[0], self.cond_channels_list[-1], bias=False)
    ...
    def forward(self, 
        ...
        '''
        node_output = super().forward(node_input=node_input.float())
        # Gate to perserve zero such that when cond_input is zero, cond_feat is also zero. This is due to the fact that cond_gate does not use bias
        cond_feat = self.cond_func(cond_input) * self.cond_gate(cond_input) # B, (L_max + 2) * C. 
        cond_feat = cond_feat.reshape(cond_feat.shape[0], (self.lmax + 1) * 2, self.num_channels)
        scale, bias_m_0 = cond_feat.split([self.lmax + 1, self.lmax + 1], dim=1)

        scale = torch.repeat_interleave(scale, torch.arange(self.lmax + 1).to(cond_feat.device) * 2 + 1, dim=1)
        scale = 1. + scale # Add one such that when cond_input is zero, the scale is 1
...
class EquivariantRMSNormArraySphericalHarmonicsV2InvCond(EquivariantRMSNormArraySphericalHarmonicsV2):
    '''
        1. Normalize across all m components from degrees L >= 0.
        2. Expand weights and multiply with normalized feature to prevent slicing and concatenation.
    '''
    ...
    def __init__(self, lmax, num_channels, mappingReduced, cond_channels_list, eps=1e-5, affine=True, normalization='component', centering=True, std_balance_degrees=True):
        ...
        # self.cond_channels_list = [50 * 160, 128, 128]
        self.cond_channels_list = cond_channels_list # [160, 128, 128 * (self.lmax + 1) * 2]
        self.cond_channels_list[-1] = (self.lmax + 2) * self.num_channels
    ...
    def forward(self, 
        ...
        node_output = super().forward(node_input=node_input.float())
        cond_feat = self.cond_func(cond_input) # B, (L_max + 2) * C
        cond_feat = cond_feat.reshape(cond_feat.shape[0], self.lmax + 2, self.num_channels)
        scale, bias_0 = cond_feat.split([self.lmax + 1, 1], dim=1)

        scale = torch.repeat_interleave(scale, torch.arange(self.lmax + 1).to(cond_feat.device) * 2 + 1, dim=1)
        bias_pad =  torch.zeros(bias_0.shape[0], node_output.shape[1] - 1, bias_0.shape[2]).to(bias_0.device)
        bias_all = torch.cat([bias_0, bias_pad], dim=1)

        output = node_output * scale[cond_batch] + bias_all[cond_batch]
