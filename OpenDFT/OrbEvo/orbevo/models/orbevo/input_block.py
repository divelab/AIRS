...
class EfieldEmbedding(torch.nn.Module):
    """
    ...
    def forward(
        ...
        x_m_0 = self.rad_func(x)
        x_m_0 = x_m_0.reshape(-1, self.m_0_num_coefficients, self.sphere_channels)
        x_m_pad = torch.zeros((
            x_m_0.shape[0], 
            (self.m_all_num_coefficents - self.m_0_num_coefficients), 
            self.sphere_channels), 
            device=x_m_0.device)
        x_m_all = torch.cat((x_m_0, x_m_pad), dim=1)

        x_embedding = SO3_Embedding(
            0, 
            self.lmax_list.copy(), 
            self.sphere_channels, 
            device=x_m_all.device, 
            dtype=x_m_all.dtype
        )
        x_embedding.set_embedding(x_m_all)
        x_embedding.set_lmax_mmax(self.lmax_list.copy(), self.mmax_list.copy())

        # Reshape the spherical harmonics based on l (degree)
        x_embedding._l_primary(self.mappingReduced)
