...
class PairNetLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.linear_node_c = Linear(
            irreps_in=self.irrep_in_node,
            irreps_out=self.irrep_tp_in_node,
            internal_weights=True,
            shared_weights=True,
        )
        self.linear_node_n = Linear(
            irreps_in=self.irrep_in_node,
            irreps_out=self.irrep_tp_in_node,
            internal_weights=True,
            shared_weights=True,
        ...
        self.tp_node_pair_msg = TensorProduct(
            self.irrep_tp_in_node,
            self.sh_irrep,
            self.irrep_tp_out_node_pair_msg,
            instruction_node_pair_msg,
            shared_weights=False,
            internal_weights=False,
        )

        self.tp_node_pair_update = FullyConnectedTensorProduct(
            self.irrep_tp_in_node,
            self.sh_irrep,
            self.irrep_tp_out_node_pair,
        )
        ...
        # self.fc_node_pair = nn.Linear(self.edge_attr_dim, self.tp_node_pair.irreps_out.dim, bias=True)
        self.fc_node_pair_msg = FullyConnectedNet(
            [self.edge_attr_dim] + invariant_layers * [invariant_neurons] + [self.tp_node_pair_msg.weight_numel],
            self.nonlinear_layer
        )
        ...
        self.model_nonlinear_layer = Activation(self.irrep_out, [
            get_nonlinear(self.nonlinear_scalars[ir.p]) if ir.l == 0 else None for _, ir in self.irrep_out])
        ...
        irrep_scalars = o3.Irreps([(mul, ir) for mul, ir in self.irrep_out if ir.l == 0])
        irrep_gated = o3.Irreps([(mul, ir) for mul, ir in self.irrep_out if ir.l > 0])
        irrep_layer_out = (irrep_scalars + irrep_gated).simplify()
        irrep_gates = o3.Irreps([(mul, "0e") for mul, _ in irrep_gated])

        self.nonlinear_node_gate = Gate(
            irreps_scalars=irrep_scalars,
            act_scalars=[get_nonlinear(self.nonlinear_scalars[ir.p]) for _, ir in irrep_scalars],
            irreps_gates=irrep_gates,
            act_gates=[get_nonlinear(self.nonlinear_gates[ir.p]) for _, ir in irrep_gates],
            irreps_gated=irrep_gated,
        )
        self.nonlinear_node_pair_gate = Gate(
            irreps_scalars=irrep_scalars,
            act_scalars=[get_nonlinear(self.nonlinear_scalars[ir.p]) for _, ir in irrep_scalars],
            irreps_gates=irrep_gates,
            act_gates=[get_nonlinear(self.nonlinear_gates[ir.p]) for _, ir in irrep_gates],
        ...
        conv_irrep_out = self.nonlinear_node_gate.irreps_in.simplify()
...
class SelfNetLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.layer_l0 = FullyConnectedNet(
            [self.irrep_in_node[0][0] * 2] + invariant_layers * [invariant_neurons] + [self.tp.weight_numel],
            self.nonlinear_layer
        )

        self.fc_node = FullyConnectedNet(
            [self.node_attr_dim] + invariant_layers * [invariant_neurons] + [self.tp.weight_numel],
            self.nonlinear_layer
        )
    ...
    def forward(self, data, x, old_fii):
        ...
        # xl = self.linear_node_1(x)
        # xr = self.linear_node_2(x)
        x = self.tp(xl, xr)
        # x = self.tp(xl, xr, self.fc_node(data.node_attr) *  self.layer_l0(torch.cat([
        #     xl[:, self.irrep_in_node.slices()[0]], xr[:, self.irrep_in_node.slices()[0]]], dim=-1)))
        # x = self.nonlinear_node_gate(x)
        if self.resnet:
            x = x + old_x
        x = self.norm_gate(x)
...
class QHNet(nn.Module):
    ...
    def forward(self, data):
        node_attr, edge_index, rbf_new, edge_sh = self.build_graph(data, self.max_radius)
        ...
        _, full_edge_index, full_edge_attr, full_edge_sh = self.build_graph(data, self.max_radius)
        ...
        for layer_idx, layer in enumerate(self.e3_gnn_layer):
            ...
            if layer_idx > 2:
                fii = self.e3_gnn_node_layer[layer_idx](data, node_attr, fii)
                fij = self.e3_gnn_node_pair_layer[layer_idx](data, node_attr, fij)
        ...
        node_pair_embedding = torch.cat([data.node_attr[dst], data.node_attr[src]], dim=-1)
        ...
        hamiltonian_matrix = self.build_final_matrix(data, hamiltonian_diagonal_matrix, hamiltonian_non_diagonal_matrix)
    ...
    def build_graph(self, data, max_radius):
        ...
        rbf = self.distance_expansion(edge_vec.norm(dim=-1).unsqueeze(-1)).squeeze()

        edge_sh = o3.spherical_harmonics(
            self.sh_irrep, edge_vec[:, [1, 2, 0]], normalize=True, normalization='component')

        return node_attr, radius_edges, rbf, edge_sh
