...
from .CoDA_utils.network import HyperEnvNet, GroupConvMLP

class CoDA(torch.nn.Module, ModelClass):
    def __init__(
            self,
            y_channels: int,
            W_channels: int,
    ...
    def forward(self, y, input_length, **kwargs):
        ...
        if self.training:
            return dydt, disentangled_params['f_e']
        else:
            return dydt
...
class HyperNetwork(torch.nn.Module):
    def __init__(self, main_func, y_channels, func_embedding_channels, hidden_channels, hyper_mlp_depth, W_channels, num_envs):
        ...
        # --- According to the CoDA paper, the dimension of epsilon^e must be small, at most 2.
        dim_epsilon_e = 2
        self.low_dim_projection = torch.nn.Linear(func_embedding_channels, dim_epsilon_e)

        self.f_c = torch.nn.Parameter(torch.randn(self.num_param_main_func))

        self.hyper_mlp = make_mlp(dim_epsilon_e, self.num_param_main_func, hidden_channels, hyper_mlp_depth)


    def forward(self, y, params_dict, rebuild_forecaster, **kwargs):
        zs = self.encoder(y) # y: B x T x y_channels
        final_z = zs[:, 0, :] # use the first token as the special summarization
        epsilon_e = self.low_dim_projection(final_z)
        f_e = self.hyper_mlp(epsilon_e)  # d_z -> d_\theta
        if not self.training:
            assert rebuild_forecaster
            assert kwargs.get('forecast_ode') is not None
            if kwargs.get('forecast_ode') == 'inv':
                params_func = self.f_c
            elif kwargs.get('forecast_ode') == 'combine':
                params_func = self.f_c + f_e
        else:
            params_func = self.f_c + f_e
        ...
        return params_dict, {'f_c': self.f_c, 'f_e': f_e}
