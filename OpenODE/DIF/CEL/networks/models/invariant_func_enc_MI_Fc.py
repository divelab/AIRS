...
class InvariantFuncEncMIFc(torch.nn.Module, ModelClass):
    def __init__(
            self,
            y_channels: int,
            W_channels: int,
    ...
    def forward(self, y, input_length, **kwargs):
        ...
        if self.training:
            ...
            pred_E = self.E_discriminator(f_c_detached)
...
class HyperNetwork(torch.nn.Module):
    ...
    def forward(self, y, params_dict, rebuild_forecaster, **kwargs):
        ...
        return params_dict, {'f_c': f_c} # 'f_e^theta': f_e_theta, 'f_e^alpha': f_e_alpha,
