...
class InvariantFuncEncMI(torch.nn.Module, ModelClass):
    def __init__(
            self,
            y_channels: int,
            W_channels: int,
    ...
    def forward(self, y, input_length, **kwargs):
        ...
        if self.training or rebuild_forecaster:
            ...
            # TODO: Do we need to constrain the training input_length?
        ...
        if self.training:
            # -- For min I(F_C;E), max I(F_E^theta;E) --
            f_c_detached = disentangled_params['f_c'].detach()
            # Not to maximize I(F_C;E)
            if self.lambda_e_detach:
                all_f = rearrange([f_c_detached, # disentangled_params['f_e^theta'].detach(), disentangled_params['f_e^alpha'].detach(),
                                   disentangled_params['f_e'].detach()],
                                  "Fc_Fe B d_z -> (Fc_Fe B) d_z")
            else:
                all_f = rearrange([f_c_detached, # disentangled_params['f_e^theta'], disentangled_params['f_e^alpha'],
                                   disentangled_params['f_e']],
                                       "Fc_Fe B d_z -> (Fc_Fe B) d_z")
            pred_E = self.E_discriminator(all_f)
            pred_E_Fc_adv = self.E_discriminator(disentangled_params['f_c'])
            # W_inv_reverse = GradientReverseLayerF.apply(W_inv, kwargs.get('alpha') * kwargs.get('lambda_reverse'))
            return dydt, disentangled_params, pred_E, pred_E_Fc_adv
        else:
            return dydt
...
class HyperNetwork(torch.nn.Module):
    ...
    def forward(self, y, params_dict, rebuild_forecaster, **kwargs):
        ...
        if not self.training:
            assert rebuild_forecaster, "During inference time, we only run the hypernetwork once for building the forecaster."
