...
class InvariantFuncEncMIF(torch.nn.Module, ModelClass):
    def __init__(
            self,
            y_channels: int,
            W_channels: int,
...
class HyperNetwork(torch.nn.Module):
    def __init__(self, main_func, y_channels, func_embedding_channels,
        ...
        self.fe_projection = make_mlp(func_embedding_channels, func_embedding_channels, hidden_channels,
                                      hyper_mlp_depth)
    ...
    def forward(self, y, params_dict, rebuild_forecaster, **kwargs):
        ...
        f = f_c + f_e
        params_func = self.hyper_mlp(f) # d_z -> d_\theta
        ...
        return params_dict, {'f': f} # 'f_e^theta': f_e_theta, 'f_e^alpha': f_e_alpha,
