...
class InvariantFuncEncMIExperiment(ExpClass):
    ...
    def __init__(self, dataloader: MyDataLoader, model: ModelClass,
                 ...
                 lambda_dis: Union[float, List[float]] = 1.0, inference_out: str = 'inv', inv_type: str = 'VREx',
                 lambda_adv: float = 1.0,
                 lambda_fet_ent: float = 1.0, lambda_pred_f: float = 1.0, lambda_pred_fc: float = 1.0, temp: float = 1.0,
                 lambda_mi: float = 1.0, mean_mi: float = 1.0, control_mi: bool = False,
                 input_length_factor: int = None, full_train_input_length: bool = False,
                 only_train_disen_projection: bool = False, use_alpha: bool = False, adapt_steps: int = 10,
        ...
        self.temp = temp
        self.lambda_pred_f = lambda_pred_f
        self.lambda_adv = lambda_adv
        self.lambda_dis = lambda_dis
        self.lambda_fet_ent = lambda_fet_ent
        self.lambda_pred_fc = lambda_pred_fc
        self.only_train_disen_projection = only_train_disen_projection
        self.use_alpha = use_alpha
        self.lambda_mi = lambda_mi
        self.mean_mi = mean_mi
        self.control_mi = control_mi

        # --- input length ablation ---
        if input_length_factor is not None:
            nT = self.loader['train'].dataset.t.shape[0]
            self.loader['train'].dataset.input_length = int(nT // input_length_factor)
            self.loader['id_test'].dataset.input_length = int(nT // input_length_factor)
            self.loader['ood_test'].dataset.input_length = int(nT // input_length_factor)
        if full_train_input_length:
            self.loader['train'].dataset.input_length = self.loader['train'].dataset.t.shape[0]
    ...
    def inv_traj_inference(self, data, input_length, forecast_ode: Literal['inv', 'combine'], invariant_state_test: bool):
        ...
        with torch.no_grad():
            ...
            try:
                # FIXME: For invariant state test, how about test in different environments?
                #        For example, some environment inputs are easier to deduce the hyperparameters.
                tik = time.time()
    ...
    def train_batch(self, data, input_length, run_type) -> dict:
        r"""
        ...
        # --- Sample pairs ---
        loss = {}
        # TODO: will uniform environment sampling provide better results?
        pred_dydt, disentangled_params, pred_E, pred_E_Fc_adv = self.model(states, input_length)  # , lambda_reverse=self.lambda_reverse)
        # pred_loss = F.mse_loss(pred_dydt, repeat(dydt, 'B T d_y -> (repeat B) T d_y', repeat=2))

        if self.inv_type == 'Invariant':
            # -- max I(F_C;Y), max I(F;Y) --
            pred_fc, pred_f = rearrange(pred_dydt, "(fc_f B) T d_y -> fc_f B T d_y", fc_f=2)
            pred_fc_loss = F.mse_loss(pred_fc, dydt)
            pred_f_loss = F.mse_loss(pred_f, dydt)

            if self.control_mi:
                mi_reg = (pred_fc_loss - self.mean_mi).relu()
                self.lambda_pred_fc = 1e-6

            # -- min I(F_C; E) = max H(E|F_C) : Compare pred_E_Fc with random (the entropy maximized) E [1/#env, 1/#env, ...] --
            # Make the discriminator more confident by using temperature less than 1.
            I_Fc_E_loss = F.cross_entropy(pred_E_Fc_adv / self.temp, torch.ones(len(env), self.model.num_envs,
                                                                    device=self.device) / self.model.num_envs)

            # -- train environment discriminator and max I(F_E^theta ; E) --
            pred_E_Fc, pred_E_Fe = rearrange(pred_E, "(Fc_Fe B) envs -> Fc_Fe B envs", Fc_Fe=2) # pred_E_Fetheta, pred_E_Fealpha,
            E_disFc_loss = F.cross_entropy(pred_E_Fc, env)
            # E_disFetheta_loss = F.cross_entropy(pred_E_Fetheta, env)
            # E_disFealpha_loss = F.cross_entropy(pred_E_Fealpha, env)
            E_disFe_loss = F.cross_entropy(pred_E_Fe, env)


            # -- min H(F_E^theta | E) --
            # H_fet_cond_E = scatter_std(disentangled_params['f_e^theta'], env, dim=0, unbiased=False).square()
            # if (H_fet_cond_E == 0).sum() > 0:
            #     raise ValueError('part of env_p_var is zero')
            # H_fet_loss = H_fet_cond_E.mean()
            if self.use_alpha:
                adversarial_loss = {'adv': {'weight': self.lambda_adv * self.alpha, 'value': I_Fc_E_loss}}
            else:
                adversarial_loss = {'adv': {'weight': self.lambda_adv * self.lambda_pred_fc, 'value': I_Fc_E_loss}}
                # adversarial_loss = {'adv': {'weight': self.lambda_adv, 'value': I_Fc_E_loss}}
            self.log_loss(loss, {key: loss_term['value'].item() for key, loss_term in adversarial_loss.items()})
            training_loss = {'pred_f': {'weight': self.lambda_pred_f, 'value': pred_f_loss},
                             'pred_fc': {'weight': self.lambda_pred_fc, 'value': pred_fc_loss},
                             'E_disFe': {'weight': self.lambda_dis, 'value': E_disFe_loss},
                             # 'E_disFetheta': {'weight': self.lambda_dis, 'value': E_disFetheta_loss},
                             # 'E_disFealpha': {'weight': self.lambda_dis, 'value': E_disFealpha_loss},
                             'E_disFc': {'weight': self.lambda_dis, 'value': E_disFc_loss},
                             # 'fet_ent': {'weight': self.lambda_fet_ent, 'value': H_fet_loss}
                             }
            if self.control_mi:
                training_loss['mi_reg'] = {'weight': self.lambda_mi, 'value': mi_reg}
                training_loss['pred_fc']['weight'] = 0.0
        else:
        ...
        if run_type == 'train':
            ...
            self.loss_wsum(adversarial_loss).backward(retain_graph=True)
            if self.only_train_disen_projection:
                self.optimizer_other_disentangle_projection.zero_grad()
            else:
                # -- README: min_{hyper_params} I(F_C; E). Delete the min I(F_C; E) gradients on the discriminator to avoiding affecting the training of the discriminator -
    ...
    def __call__(self, *args, **kwargs):
        ...
        other_params = set(self.model.parameters()) - set(self.model.hyper_network.fc_projection.parameters())
        self.optimizer_other_disentangle_projection = torch.optim.Adam(other_params, lr=self.lr)
        # - only used to clear the gradients of the environment discriminator -
        self.optimizer_discriminator = torch.optim.Adam(self.model.E_discriminator.parameters(), lr=self.lr)
        ...
        for epoch in range(self.ctn_epoch, self.max_epoch + 1):
            ...
            if epoch % 100 == 0:
                # Use training data to calculate the loss again instead of using the mean training loss as the indicator.
                runs = ['train', 'id_test']
            else:
                runs = ['train']
            for run_type in runs:
                if run_type == 'id_test':
                    print('#IN#Evaluate losses on id_test')
                pbar = enumerate(self.loader[run_type]) if self.wandb_logger else tqdm(enumerate(self.loader[run_type]),
                                                                                      total=len(self.loader[run_type]),
                                                                                      **self.pbar_setting)
                for index, traj_batch in pbar:
                    ...
                    # Parameter for DANN
                    p = (index / len(self.loader[run_type]) + epoch) / self.max_epoch
                    self.alpha = 2. / (1. + np.exp(-10 * p)) - 1

                    # train a batch
                    try:
                        with torch.set_grad_enabled(run_type == 'train'):
                            train_loss = self.train_batch(traj_batch, input_length=self.loader[run_type].dataset.input_length, run_type=run_type)
            ...
            if epoch % 100 == 0:
                ...
                if epoch % self.log_interval == 0 and self.wandb_logger:
                    wandb.log({
                        ...
                        'Best_inv_train_loss': self.best_inv_train_loss,
                        'Conditional_MI': self.best_inv_train_loss['pred_fc'] - self.best_inv_train_loss['pred_f'],
                        'True_Conditional_MI': 0.5 * np.log(self.best_inv_train_loss['pred_fc'] / self.best_inv_train_loss['pred_f']),
    ...
    def explain_in_pysr(self, exp_hash, epoch=None):
        ...
        scaling = self.loader['id_test'].dataset.max_for_scaling
        ...
        def model_fit_sym(data, input_length=1, forecast_ode='inv'):
            ...
            unscaled_inv_states = invariant_states * scaling # unscale
            yinv_W, ps = pack([unscaled_inv_states, W], 'bs T *') # bs T (Cy + Cw)
            ...
            predicted_dy = pred_dy.cpu().detach()
            unscaled_pred_dy = predicted_dy * scaling # unscale
            sr_model.fit(rearrange(yinv_W, 'bs T C -> (bs T) C').numpy(),
                         rearrange(unscaled_pred_dy, 'bs T Cy -> (bs T) Cy').numpy())
            return sr_model
    ...
    def report_evaluation(self, best_error, epoch, mean_error, split, ode_type, **kwargs):
        if mean_error < best_error:
            ...
            if ode_type == 'inv' and split == 'id_test_invariant_states':
                self.best_inv_train_loss = kwargs['train_mean_loss']
