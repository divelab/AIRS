...
class MAMLExperiment(ExpClass):
    ...
    def __init__(self, dataloader: MyDataLoader, model: ModelClass,
                 ...
                 inference_out: str = 'inv', inv_type: str = 'VREx',
                 adapt_steps: int = 10,
    ...
    def train_batch(self, data, input_length) -> dict:
        ...
        meta_dydt = task_model(states, input_length)
        adaptation_loss = F.mse_loss(meta_dydt, dydt)

        task_model.adapt(adaptation_loss)

        adapted_dydt = task_model(states, input_length)
        evaluation_loss = F.mse_loss(adapted_dydt, dydt)

        evaluation_loss.backward()
        self.optimizer.step()

        training_loss = {'adaptation_loss': {'weight': 1.0, 'value': adaptation_loss}, 'evaluation_loss': {'weight': 1.0, 'value': evaluation_loss}}
