...
class InvariantExperiment(ExpClass):
    ...
    def inv_traj_inference(self, data, input_length):
        ...
        Wc = data['Wc'].to(self.device)
        t = data['t'].to(self.device)[0]

        def inv_forward_ode_func(t, y):
            pred_y, inv_dy = self.model(y, Wc)
            return inv_dy
    ...
    def train_batch(self, data) -> dict:
        ...
        for idx in sample_ids:
            pred_dydt, inv_dydt = self.model(states[arange_idx, idx], Wc, alpha=self.alpha, lambda_reverse=self.lambda_inv)  # Select (#0, idx[0]), (#1, idx[1]), ...
            ...
            sum(loss_term['weight'] * loss_term['value'] for loss_term in training_loss.values()).backward()
            self.optimizer.step()
