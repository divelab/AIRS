...
class InvariantFuncEncMISlowExperiment(ExpClass):
    ...
    def train_batch(self, data, input_length, run_type) -> dict:
        ...
        tik = time.time()
        for i in range(200):
            pred_dydt = self.model(states, input_length)  # , lambda_reverse=self.lambda_reverse)
            time_array.append(time.time() - tik)
        print(f'Time: {time.time() - tik:.2f}')
        time_array = np.array(time_array)
        np.save(pathlib.Path(os.environ['ROOT_DIR']) / f'vectorize{self.model.vectorize_func}_assign{self.model.always_rebuild_forecaster}_deepcopy{self.model.deep_copy}.npy', time_array)
        exit()
                ...
                for i in range(10):
                    ...
                    for index, traj_batch in pbar:
                        ...
                        try:
                            ...
                            print(e)
                    print(f'#IN#{i}: {time.time() - tik:.2f}s')
                exit()
