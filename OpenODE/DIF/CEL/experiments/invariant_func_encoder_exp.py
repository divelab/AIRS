...
class InvariantFuncEncExperiment(ExpClass):
    ...
    def __init__(self, dataloader: MyDataLoader, model: ModelClass,
                 ...
                 lambda_inv: Union[float, List[float]] = 1.0, inference_out: str = 'inv', inv_type: str = 'VREx', lambda_env: float = 1.0,
                 lambda_env_dis: float = 1.0, lambda_reverse: float = 1.0, adapt_steps: int = 10, adapt_lr: float = 1e-2, **kwargs):
        r'''
        ...
        self.lambda_env_dis = lambda_env_dis
    ...
    def inv_traj_inference(self, data, input_length):
        ...
        self.model(states, input_length)
        self.count = 0
    ...
    def train_batch(self, data, input_length) -> dict:
        ...
        pred_dydt, disentangled_params, W_env_pred = self.model(states, input_length, alpha=self.alpha, lambda_reverse=self.lambda_reverse)
        pred_loss = F.mse_loss(pred_dydt, repeat(dydt, 'B T d_y -> (repeat B) T d_y', repeat=2))
        if self.inv_type == 'VREx':
            inv_p_var = reduce(disentangled_params['inv'], 'B p -> p', torch.var)
            inv_p_loss = inv_p_var.mean()
            env_p_var = scatter_std(disentangled_params['env'], env, dim=0, unbiased=False).square()
            if (env_p_var == 0).sum() > 0:
                raise ValueError('part of env_p_var is zero')
            env_p_loss = env_p_var.mean()

            env_d_loss = F.cross_entropy(W_env_pred, env)
            # assert not torch.isnan(inv_p_var).any() and not torch.isnan(env_p_var).any() and not torch.isinf(inv_p_var).any() and not torch.isinf(env_p_var).any()
            training_loss = {'pred': {'weight': 1.0, 'value': pred_loss},
                             'inv': {'weight': self.lambda_inv, 'value': inv_p_loss},
                             'env': {'weight': self.lambda_inv, 'value': env_p_loss}, # Share the same weight
                             'dis_env': {'weight': self.lambda_env_dis, 'value': env_d_loss}}
        # if self.inv_type == 'VREx':
    ...
    def __call__(self, *args, **kwargs):
        ...
        # self.optimizer = torch.optim.Adam(self.model.hyper_network.parameters(), lr=self.lr,
        #                                   weight_decay=self.weight_decay)
