...
class CoDAExperiment(ExpClass):
    ...
    def __init__(self, dataloader: MyDataLoader, model: ModelClass,
                 ...
                 lambda_reg: Union[float, List[float]] = 1.0, inference_out: str = 'inv', inv_type: str = 'VREx',
                 adapt_steps: int = 10,
    ...
    def train_batch(self, data, input_length) -> dict:
        ...
        pred_dydt, f_e = self.model(states, input_length)
        pred_loss = F.mse_loss(pred_dydt, dydt)

        if self.inv_type == 'Invariant':
            reg_fe = (torch.norm(f_e, dim=0) ** 2).sum()

            training_loss = {'pred': {'weight': 1.0, 'value': pred_loss},
                             'reg_fe': {'weight': self.lambda_reg, 'value': reg_fe}}
        else:
            pred_loss = F.mse_loss(pred_dydt, dydt)
            training_loss = {'pred': {'weight': 1.0, 'value': pred_loss}}
