...
class HyenaDNAModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_layer: int, d_inner: int, vocab_size: int,
                 ...
                 resid_dropout: float = 0.0, embed_dropout: float = 0.1,
                 layer_norm_epsilon: float = 1e-5, initializer_cfg=None,residual_in_fp32=False,
                 pad_vocab_size_multiple: int = 1, use_head=False, n_classes: int = 2, next_token=False,
                 device=None, dtype=None, **kwargs) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
...
class HyenaDNAPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls,
        ...
        scratch_model = HyenaDNAModel(**config, use_head=use_head, n_classes=n_classes, next_token=next_token)  # the new model format
