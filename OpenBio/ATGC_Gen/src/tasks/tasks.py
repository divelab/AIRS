...
class GenTask(BaseTask):
    def _reset_torchmetrics(self, prefix=None):
        torchmetrics = getattr(self, f'{prefix}_torchmetrics')
        torchmetrics.reset()

    def forward(self, batch, encoder, model, decoder, _state):
        data, target, signal = batch['data'], batch['target'], batch['condition']
        return_dict = {"protein_name": batch['protein_name']} if "protein_name" in batch else {}
        cell_type_input = batch['cell_type'] if 'cell_type' in batch else None
        logits = self.model(data, condition=signal, condition2=cell_type_input)  # bs * seq * char len

        return logits.contiguous(), target, return_dict


class MaskGenTask(BaseTask):
    def forward(self, batch, encoder, model, decoder, _state):
        input_embeds, condition, input_seqs = batch
        condition = None if condition.numel() == 0 else condition
        logits, label = self.model(input_embeds, input_seqs, condition=condition)
        return logits.contiguous(), label.contiguous(), {}
