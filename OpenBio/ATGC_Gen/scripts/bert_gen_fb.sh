...
ROOT_PATH=""
model_path="$ROOT_PATH/outputs/enhancers_fb/train_inf_bert_base_step1_lr1e-4/checkpoints/val/fbd.ckpt"


python -m train \
    experiment=hg38/enhancer_flybrain_debug \
    wandb.mode=online \
    wandb.group=Enhancer_Fb \
    wandb.name=genfull_bert_base_temp \
    hydra.run.dir="./outputs/enhancers_fb/genfull_bert_base_temp" \
    task=DNA_masked_gen \
    model=bert \
