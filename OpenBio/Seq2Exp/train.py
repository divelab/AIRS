...
from src.models.sequence.GeneExpformer import entropy
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def reset_torch_metrics(self):
        # val
        self.all_preds_val, self.all_labels_val = [], []
        ...
        # test
        self.all_preds_test, self.all_labels_test = [], []
        self.sample_count_test = torch.tensor(0, dtype=torch.int64)
        # mask
        self.mask_count_test = []
        self.all_mask_test = []
    ...
    def _shared_step(self, batch, batch_idx, prefix="train"):
        ...
        if self.hparams.task._name_ in ['pe_inter', 'gene_expression', 'extract_rationale']:
            cur_batch_size = len(batch[-1])
        elif self.hparams.task._name_ == 'gnn':
        ...
        # Loss
        if prefix == 'train':
            loss_dict = self.loss(x, y, **w)
        else:
            loss_dict = self.loss_val(x, y, **w)
        ...
        if 'mask' in w:
            mask_entropy = entropy(w['mask'])
            metrics['mask_entropy'] = mask_entropy
        ...
        if prefix != 'train':
            ...
            if 'mask' in w:
                mask_learn = w['mask']
                avg_mask_count = torch.sum(mask_learn, dim=1)
                avg_mask_count = self.device_gather(avg_mask_count)
                # if self.trainer.is_global_zero:
                mask_count = getattr(self, f"mask_count_{prefix}")
                mask_count += list(avg_mask_count.detach().cpu().numpy())
                if self.hparams.train.only_test:
                    # record mask to see the mask distribution
                    all_mask = getattr(self, f"all_mask_{prefix}")
                    all_mask += mask_learn.detach().cpu().tolist()

            x = self.device_gather(x)
            y = self.device_gather(y)

            # if self.trainer.is_global_zero:
            outputs = list(x.flatten().detach().cpu().numpy())
            labels = list(y.flatten().detach().cpu().numpy())

            all_preds = getattr(self, f"all_preds_{prefix}")
            all_preds += outputs
            all_labels = getattr(self, f"all_labels_{prefix}")
            all_labels += labels
            sample_count = getattr(self, f"sample_count_{prefix}")
            sample_count.add_(cur_batch_size)
    ...
    #             saved_grads[name] = param.grad.clone().detach()
    #             if torch.isnan(param.grad).any():
    #                 print(f"NaN found in gradients of layer: {name}")
    #                 nan_in_grad = True
    #
    #     if nan_in_grad:
    #         print(f"NaN detected in gradients after backward in epoch {self.current_epoch}")

    def eval_metrics(self, prefix):
        all_preds = getattr(self, f"all_preds_{prefix}")
        all_labels = getattr(self, f"all_labels_{prefix}")
        mask_count = getattr(self, f"mask_count_{prefix}")
        ...
        if len(all_preds) == 0:
            r_value = peasonr = 0.0
            mse = mae = 1.0
            avg_mask_num = avg_mask_percent = 0
            return {}
        else:
            #
            slope, intercept, r_value, p_value, std_err = stats.linregress(all_preds, all_labels)
            peasonr, pvalue = stats.pearsonr(all_preds, all_labels)
            mse = mean_squared_error(all_preds, all_labels)
            mae = mean_absolute_error(all_preds, all_labels)
            # mask
            avg_mask_num = np.mean(mask_count) if len(mask_count) > 0 else 0
            avg_mask_percent = avg_mask_num / self.hparams.dataset.seq_range

        metrics = {
            f'{prefix}/mse': torch.tensor(mse, dtype=torch.float32),
            f'{prefix}/mae': torch.tensor(mae, dtype=torch.float32),
            f'{prefix}/r_square': torch.tensor(r_value**2, dtype=torch.float32),
            f'{prefix}/peasonr': torch.tensor(peasonr, dtype=torch.float32),
            f'{prefix}/avg_num_mask': torch.tensor(avg_mask_num, dtype=torch.float32),
            f'{prefix}/avg_percent_mask': torch.tensor(avg_mask_percent, dtype=torch.float32),
        }
        return metrics
    ...
    def eval_epoch_end(self, prefix):
        if self.hparams.train.only_test:
            self.all_mask_val
        # if self.trainer.is_global_zero:
        if (not self.hparams.train.remove_test_loader_in_eval) and (not self.hparams.train.remove_val_loader_in_eval):
            val_metrics = self.eval_metrics('val')
            test_metrics = self.eval_metrics('test')
            metrics = {**val_metrics, **test_metrics}
        else:
            metrics = self.eval_metrics(prefix)

        # Calculate torchmetrics
        # torchmetrics = getattr(self, f'{prefix}_torchmetrics')
        # custom_metrics = torchmetrics(x, y, loss=loss)
        self.log_dict(
            metrics,
            # on_step=log_on_step,
            # on_epoch=True,
    ...
    def validation_epoch_end(self, outputs):
        # outputs will contain the output for each validation_step dataloader_idx, form a list
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        loss_epoch = {
            # "trainer/loss": loss,
            "trainer/epoch": float(self.current_epoch),
        }
        self.log_dict(
            loss_epoch,
            on_step=False,
...
def train(config):
    # set number of thread in system
    # max_threads = config.sys_set.num_threads
    # if max_threads > 0:
    ...
    if config.train.test:
        ...
        test_results = model_test(config, best_val_ckpt)
        return test_results
...
def one_fold_training(fold, ori_wandb_name, ori_wandb_id, config_best_model_path, config):
    fold_config = copy.deepcopy(config)

    fold_config.wandb.name = ori_wandb_name + f'+Fold{fold}'
    fold_config.wandb.id = ori_wandb_id + f'+Fold{fold}'
    fold_config.callbacks.model_checkpoint.filename = config_best_model_path + f'+Fold{fold}'
    fold_config.dataset.k_fold = fold
    cur_test_result = train(fold_config)

    return cur_test_result
...
def main(config: OmegaConf):
    ...
    if config.train.only_test:
        config.dataset.k_fold = config.train.single_CV
        model_test(config, config.train.only_test_model_path, evaluate=config.train.evaluate)
    else:
        ori_wandb_name = config.wandb.name
        ori_wandb_id = config.wandb.id
        config_best_model_path = config.callbacks.model_checkpoint.filename
        test_monitor = config.train.monitor.replace('val', 'test')
        if config.train.single_CV:
            cur_test_result = one_fold_training(config.train.single_CV, ori_wandb_name, ori_wandb_id, config_best_model_path, config)
            final_test_result = cur_test_result[test_monitor]
            if wandb.run is not None:
                wandb.finish()
            log.info(f'final single cross-validation result for CV={config.train.single_CV} is {test_monitor} = {final_test_result}')
        elif config.train.num_fold_CV:
            all_test_result = []
            if isinstance(config.train.num_fold_CV, int):
                iterate = range(config.train.num_fold_CV)
            elif isinstance(config.train.num_fold_CV, str):
                iterate = config.train.num_fold_CV.split(',')
            for fold in iterate:
                fold = int(fold)
                cur_test_result = one_fold_training(fold + 1, ori_wandb_name, ori_wandb_id, config_best_model_path, config)
                all_test_result.append(cur_test_result[test_monitor])
                if wandb.run is not None:
                    wandb.finish()
            log.info(f'each {test_monitor} = {all_test_result}')
            log.info(f'final cross-validation average results is {test_monitor} = {np.mean(all_test_result)}')
        else:
            train(config)
