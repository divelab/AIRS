...
def poisson_loss(inputs, target, log_input=False, full=True):
    return F.poisson_nll_loss(inputs, target, log_input=log_input, full=full)


def smooth_l1_loss(inputs, target):
    return {'pred_loss': F.smooth_l1_loss(inputs, target)}


def extract_rationale_loss(
        inputs, target,
    ...
    loss_dict = {'pred_loss': F.smooth_l1_loss(inputs, target)}
    if select_loss is not None and select_weight != 0:
        loss_dict['select_loss'] = select_weight * select_loss
    if continuity_loss is not None and continuity_weight != 0:
        loss_dict['continuity_loss'] = continuity_weight * continuity_loss
    if bio_mask_loss is not None and bio_mask_weight != 0:
        loss_dict['bio_mask_loss'] = bio_mask_weight * bio_mask_loss
    if kl_loss is not None and kl_loss_weight != 0:
        loss_dict['kl_loss'] = kl_loss_weight * kl_loss
    return loss_dict
...
output_metric_fns = {
    ...
    "extract_rationale_loss": extract_rationale_loss,
}
